...
{
    static UnityEngine.AI.NavMeshTriangulation nav;
    static Mesh mesh;
    ...
    private static int GetRandomConnectedTriangleOnNavMesh(Vector3 p)
    {
        ...
        {
            path.ClearCorners();
            if (UnityEngine.AI.NavMesh.CalculatePath(p, mesh.vertices[mesh.triangles[3 * i + 0]], UnityEngine.AI.NavMesh.AllAreas, path))
            {
